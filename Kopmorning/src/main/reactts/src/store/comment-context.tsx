...
interface CommentCtx {
  ...
  postComment: (comment:PostComment, token:string) => void;
...
export const CommentContextProvider:React.FC<Props> = (props) => {
  ...
  const postCommentHandler = async (comment:PostComment, token:string) => {
    setIsSuccess(false);
    const postData = await commentAction.postComment(comment, token);
